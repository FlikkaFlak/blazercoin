...
uint256 hashGenesisBlock("0x1040a19e0803e77f56662a6e6feb76c1edc690bbab90caf5fa36189e7ab6ba2c");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(537261531); // BlazerCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BlazerCoin Signed Message:\n";
...
{
    // BlazerCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 8400000000; // 84 Reward

	if(nHeight == 1)
            {
            nSubsidy = 56448000000000; // 564,480 Premine for Development & Bounties
   }
	else { nSubsidy = 8400000000; } // 84 Reward

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 360; // BlazerCoin: 6 minutes
static const int64 nTargetSpacing = 180; // BlazerCoin: 3 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BlazerCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
				...
				if (pindexLast->nHeight+1 >= 40) { DiffMode = 2; }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BlazerCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BlazerCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7b026df6f36ca2f3516c1026ae4615907767d780ad4277074543a74e3fdf921e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Rohrabacher bill would protect states from federal crackdowns March 26 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 8400000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("d0ad5d3f7a3b3acaed326214ac14bdc0857a17cc20b6c734472f82ec1d694777c9daabe8db248857f36c88015f851a10e3c7a2d01f38a6cd7101c95f3fccf2fea2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1490590477;
        block.nBits    = 537261531;
        block.nNonce   = 3309349819;

        if (fTestNet)
        {
            block.nTime    = 1490590477;
            block.nNonce   = 4074414301;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x8876263fededd06fc6ddb4579bf6edbc8a1f064a7508998d63cdf0b042a737e3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BlazerCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlazerCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BlazerCoinMiner : ProcessBlock, block not accepted");
    }
